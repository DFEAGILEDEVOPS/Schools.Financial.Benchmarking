...
(function (GOVUK, Views) {
  ...
  BenchmarkChartsViewModel.RefreshAddRemoveLinks = function () {
    ...
    if (scope) {
      scope.dataLoaded.then(function () {
    ...
    c3.generate({
      ...
      tooltip: {
        contents: function (d, defaultTitleFormat) {
          ...
          var tableHtml = "<table class='bmc-rollover-table' >" + "<tr><th colspan='2' class='" + highlight + "'>" + name + "</th></tr>" + "<tr><td class='bold'>Local authority</td><td>" + schoolData.la + "</td></tr>" + "<tr><td class='bold'>School type</td><td>" + schoolData.type + "</td></tr>" + "<tr><td class='bold'>Number of pupils</td><td>" + schoolData.pupilCount + "</td></tr>";
  ...
  BenchmarkChartsViewModel.GenerateCharts = function (unitParameter) {
    ...
    $(".chart").each(function () {
      ...
      var minBy = _.minBy(yValues, function (o) {
        return o.amount;
      });

      var minimum = minBy ? minBy.amount : 0;

      var maxBy = _.maxBy(yValues, function (o) {
        return o.amount;
      });
      ...
      if (minimum === 0 && maximum === 0) {
        ...
        var newMin = minimum < 0 ? range * Math.floor(minimum / range) : range * Math.round(minimum / range);
  ...
  BenchmarkChartsViewModel.RebuildCharts = function () {
    ...
    var type = $("#Type").val();
    var url = "/benchmarkcharts/getcharts?revgroup=" + tabParameter + "&showValue=" + unitParameter + "&chartGroup=" + chartGroupParameter;
  ...
  BenchmarkChartsViewModel.ChangeTab = function (tab) {
    ...
    if (tab === "Custom") {
      ...
      var formatParameter = sessionStorage.chartFormat;
      var url = "/benchmarkcharts/tabchange?tab=" + tab + "&type=" + typeParameter + "&showValue=" + unitParameter;
...
function PdfGenerator() {
  ...
  function pdfWriteTable(id) {
    var headers = $(id + ' th').toArray().map(function (th) {
    ...
    doc.table(MARGIN_LEFT, offset, data, headers, {
      autoSize: true,
      fontSize: 9,
      margins: {
        bottom: 5,
