...
(function (GOVUK, Views) {
  ...
  SchoolsSearchViewModel.prototype = {
    bindEvents: function bindEvents() {
      ...
      this.bindAutosuggest('#FindSchoolByLaCodeName', '#SelectedLocalAuthorityId', {
    ...
    bindAccordionHeaderClick: function bindAccordionHeaderClick() {
    ...
    bindEnterKeysToButtons: function bindEnterKeysToButtons() {
    ...
    findCurrentLocationHandler: function findCurrentLocationHandler(evt) {
      evt.preventDefault();
    ...
    getSchoolsSuggestionHandler: function getSchoolsSuggestionHandler(keywords, callback) {
    ...
    getCurrentPositionErrorHandler: function getCurrentPositionErrorHandler(err) {
      var msg;
      ...
      var html = '<div class="error-summary" role="alert" aria-labelledby="ErrorSummaryHeading">' + '<h1 id = "ErrorSummaryHeading" class="heading-medium error-summary-heading">' + 'There are errors on this page that require attention.' + '</h1>' + '<ul class="error-summary-list">' + '<li>' + '<a id="error-msg" href="#finderSection">' + msg + '</a>' + '</li>' + '</ul>' + '</div>';
    ...
    getCurrentPositionSuccessHandler: function getCurrentPositionSuccessHandler(position) {
    ...
    getLocationResultsHandler: function getLocationResultsHandler(keywords, callback) {
      azureMapsClient.search.getSearchAddress(keywords, {
        ...
        if (response && response.results !== null && response.results.length > 0) {
          ...
          suggestions = results.features.filter(function (r) {
            return r.properties.address.countrySubdivision === "ENG";
          }).map(function (r) {
            var obj = {};
            var output = "";
