...
(function (GOVUK, Views) {
  ...
  AdvancedCharacteristicsViewModel.prototype = {
    ...
    updateResultCount: function updateResultCount() {
      ...
      jqxhr = $.post("GenerateCountFromManualCriteria", $('#criteriaForm').serialize()).done(function (count) {
        ...
        if (count > 0) {
          $("button.submit").show();
          $("button.submit").removeAttr("disabled"); //if (count <= 30) {
    ...
    bindEvents: function bindEvents() {
      ...
      $(questionCheckBoxSelector).change(function (event) {
        ...
        $panel.toggle();
        $panel.find("input").prop('disabled', function (i, v) {
          return !v;
        });
