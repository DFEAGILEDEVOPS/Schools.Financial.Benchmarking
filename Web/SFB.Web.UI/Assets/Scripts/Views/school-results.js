...
(function (GOVUK, Views) {
  ...
  SchoolsResultsViewModel.prototype = {
    ...
    bindTracking: function () {
      ...
      $(".js-live-search-results-block input[type=checkbox]").change(function () {
        if ($(this).is(":checked")) {
          ...
          var facetGroupName = $(this).closest("div.govuk-option-select").find("div.option-select-label").text();
    ...
    bindMap: function () {
      ...
      if (!this.mapLoaded && this.currentTabName === "map") {
        ...
        if (this.location == null) {
          this.location = {
            lat: 52.636,
            lng: -1.139
          }; // no location specified, so use central England.
      ...
      if (!this.iconBlack) {
        this.iconBlack = {
          url: "/public/assets/images/icons/icon-location.png",
          scaledSize: new google.maps.Size(20, 32)
        };
        this.iconPink = {
          url: "/public/assets/images/icons/icon-location-pink.png",
          scaledSize: new google.maps.Size(20, 32)
        };
      }
    },
    ...
    renderMapPins: function (response, serialisedState) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        var lat = new Number(data[i].Latitude),
            lng = new Number(data[i].Longitude);
        var key = genKey(lat, lng);
        if (!hashtable[key]) hashtable[key] = key;else {
          lng += adjustment;
          key = genKey(lat, lng);
          hashtable[key] = key;
        }
      ...
      this.markerCluster = new MarkerClusterer(this.map, markers, {
        imagePath: "/public/js-marker-clusterer/images/m",
      ...
      if (self.count > 1000) $(".map-view-qualifier").show();else $(".map-view-qualifier").hide();
    }
  };
  ...
  SchoolsResultsViewModel.UpdateBenchmarkBasket = function (urn, withAction) {
    ...
    $.get("/school/UpdateBenchmarkBasket?urn=" + urn + "&withAction=" + withAction, function (data) {
